"""
SICRO PCI/AP — Sistema de Croquis da Polícia Científica do Amapá
Desenvolvido por: André Ricardo Barroso — Perito Criminal
Versão: 10.0.0
"""
...
def carregar_imagem_veiculo(nome_arquivo, cor_hex):
    ...
    if chave in _IMG_CACHE:
        return _IMG_CACHE[chave]
    try:
        import numpy as np
        ...
        if alpha.min() == 255:
            ...
            threshold = 20
            h_, w_ = gray.shape
            starts = [(0,x) for x in range(0,w_,4)] + [(h_-1,x) for x in range(0,w_,4)] + [(y,0) for y in range(0,h_,4)] + [(y,w_-1) for y in range(0,h_,4)]
            from collections import deque
            queue = deque()
            for sy, sx in starts:
            ...
            arr[~mask, 3] = 0
        try:
            cr = int(cor_hex[1:3], 16)
            cg = int(cor_hex[3:5], 16)
            cb_ = int(cor_hex[5:7], 16)
        except Exception:
            cr, cg, cb_ = 79, 114, 224
        opaque = arr[:,:,3] > 30
        r2 = arr[:,:,0].astype(int)
        g2 = arr[:,:,1].astype(int)
        b2 = arr[:,:,2].astype(int)
        ...
        arr[alvo, 1] = cg
        arr[alvo, 2] = cb_
        resultado = Image.fromarray(arr)
        _IMG_CACHE[chave] = resultado
        return resultado
    except Exception as e:
        print(f"[carregar_imagem_veiculo] erro: {e}")
        return None

# ─────────────────────────────────────────────
#  CATÁLOGO DE MODELOS DE VEÍCULOS
#  Cada modelo define: chave, nome, tipo base,
#  dimensões padrão, cor padrão e função de
#  arte (recebe canvas, cx, cy, escala, cor)
# ─────────────────────────────────────────────
def _arte_sedan(c, cx, cy, sc, cor):
    """Carro sedan visto de cima — 4 portas"""
    w,h = 28*sc, 14*sc
    # Carroceria
    c.create_rectangle(cx-w/2, cy-h/2, cx+w/2, cy+h/2,
                       fill=cor, outline=BRANCO, width=1)
    # Para-brisas dianteiro
    c.create_polygon(cx-w/2+3*sc, cy-h/2,
                     cx+w/4,      cy-h/2,
                     cx+w/4,      cy-h/2+4*sc,
                     cx-w/2+2*sc, cy-h/2+4*sc,
                     fill="#A0C8F0", outline="")
    # Vidro traseiro
    c.create_polygon(cx+w/4,     cy-h/2,
                     cx+w/2-2*sc, cy-h/2,
                     cx+w/2-2*sc, cy-h/2+4*sc,
                     cx+w/4,      cy-h/2+4*sc,
                     fill="#A0C8F0", outline="")
    # Rodas (4)
    rw,rh = 3*sc, 4*sc
    for rx,ry in [(-w/2+2*sc,-h/2-rh/2),(-w/2+2*sc,h/2-rh/2),
                  ( w/2-4*sc,-h/2-rh/2),( w/2-4*sc,h/2-rh/2)]:
        c.create_rectangle(cx+rx-rw/2, cy+ry, cx+rx+rw/2, cy+ry+rh,
                           fill="#222", outline="#555", width=1)

def _arte_suv(c, cx, cy, sc, cor):
    """SUV / picape — mais largo e quadrado"""
    w,h = 30*sc, 16*sc
    # Carroceria principal
    c.create_rectangle(cx-w/2, cy-h/2, cx+w/2, cy+h/2,
                       fill=cor, outline=BRANCO, width=1)
    # Capô
    c.create_rectangle(cx-w/2, cy-h/2, cx-w/2+8*sc, cy+h/2,
                       fill=_escurecer(cor), outline="")
    # Para-brisas
    c.create_polygon(cx-w/2+8*sc,  cy-h/2+2*sc,
                     cx-w/2+14*sc, cy-h/2+2*sc,
                     cx-w/2+14*sc, cy+h/2-2*sc,
                     cx-w/2+8*sc,  cy+h/2-2*sc,
                     fill="#A0C8F0", outline="")
    # Rodas grandes
    rw,rh = 4*sc,5*sc
    for rx,ry in [(-w/2+3*sc,-h/2-rh/2),(-w/2+3*sc, h/2-rh/2),
                  ( w/2-5*sc,-h/2-rh/2),( w/2-5*sc, h/2-rh/2)]:
        c.create_rectangle(cx+rx-rw/2,cy+ry, cx+rx+rw/2,cy+ry+rh,
                           fill="#222",outline="#555",width=1)

def _arte_hatch(c, cx, cy, sc, cor):
    """Hatchback — traseira alta e curta"""
    w,h = 24*sc, 13*sc
    # Carroceria
    c.create_rectangle(cx-w/2, cy-h/2, cx+w/2, cy+h/2,
                       fill=cor, outline=BRANCO, width=1)
    # Teto (forma trapezoidal vista de cima = mais estreito)
    c.create_polygon(cx-w/2+4*sc, cy-h/2+2*sc,
                     cx+w/2-3*sc, cy-h/2+2*sc,
                     cx+w/2-3*sc, cy+h/2-2*sc,
                     cx-w/2+4*sc, cy+h/2-2*sc,
                     fill=_escurecer(cor), outline="")
    # Para-brisas curto
    c.create_polygon(cx-w/2+2*sc, cy-h/2,
                     cx-w/2+7*sc, cy-h/2,
                     cx-w/2+6*sc, cy-h/2+3*sc,
                     cx-w/2+2*sc, cy-h/2+3*sc,
                     fill="#A0C8F0", outline="")
    # Rodas
    rw,rh = 3*sc,4*sc
    for rx,ry in [(-w/2+2*sc,-h/2-rh/2),(-w/2+2*sc,h/2-rh/2),
...
def _arte_moto_esportiva(c, cx, cy, sc, cor):
    """Moto esportiva — baixa e estreita"""
    w,h = 20*sc, 7*sc
    # Tanque/carenagem central
    c.create_polygon(cx-w/2, cy,
                     cx-w/2+4*sc, cy-h/2,
                     cx+w/2-3*sc, cy-h/2+1*sc,
                     cx+w/2,      cy,
                     cx+w/2-3*sc, cy+h/2-1*sc,
                     cx-w/2+4*sc, cy+h/2,
                     fill=cor, outline=BRANCO, width=1)
    # Guidão
    c.create_line(cx-w/2+3*sc, cy-h/2-2*sc,
                  cx-w/2+3*sc, cy+h/2+2*sc,
                  fill=CINZA_CLARO, width=int(1.5*sc))
    # Rodas
    rr = 3.5*sc
    c.create_oval(cx-w/2+1*sc-rr/2, cy-rr/2,
                  cx-w/2+1*sc+rr/2, cy+rr/2,
                  fill="#222", outline=CINZA_CLARO, width=1)
    c.create_oval(cx+w/2-2*sc-rr/2, cy-rr/2,
                  cx+w/2-2*sc+rr/2, cy+rr/2,
                  fill="#222", outline=CINZA_CLARO, width=1)

def _arte_moto_urbana(c, cx, cy, sc, cor):
    """Moto urbana / naked — mais alta"""
    w,h = 18*sc, 8*sc
    # Corpo
    c.create_rectangle(cx-w/2+3*sc, cy-h/2,
                       cx+w/2-3*sc, cy+h/2,
                       fill=cor, outline=BRANCO, width=1)
    # Guidão largo
    c.create_line(cx-w/2+2*sc, cy-h/2-3*sc,
                  cx-w/2+2*sc, cy+h/2+3*sc,
                  fill=CINZA_CLARO, width=int(2*sc))
    # Espelhos
    c.create_line(cx-w/2+1*sc, cy-h/2-3*sc,
                  cx-w/2+4*sc, cy-h/2-3*sc,
                  fill=CINZA_CLARO, width=1)
    c.create_line(cx-w/2+1*sc, cy+h/2+3*sc,
                  cx-w/2+4*sc, cy+h/2+3*sc,
                  fill=CINZA_CLARO, width=1)
    # Rodas
    rr=3*sc
    c.create_oval(cx-w/2-rr/2, cy-rr/2, cx-w/2+rr/2, cy+rr/2,
                  fill="#222",outline=CINZA_CLARO,width=1)
    c.create_oval(cx+w/2-rr/2, cy-rr/2, cx+w/2+rr/2, cy+rr/2,
                  fill="#222",outline=CINZA_CLARO,width=1)

def _arte_moto_carga(c, cx, cy, sc, cor):
    """Motoboy / carga — baú traseiro"""
    w,h = 22*sc, 9*sc
    # Corpo
    c.create_rectangle(cx-w/2+4*sc, cy-h/2+1*sc,
                       cx+w/4,      cy+h/2-1*sc,
                       fill=cor, outline=BRANCO, width=1)
    # Baú traseiro
    c.create_rectangle(cx+w/4, cy-h/2,
                       cx+w/2, cy+h/2,
                       fill=CINZA_ESCURO, outline=CINZA_CLARO, width=1)
    # Guidão
    c.create_line(cx-w/2+3*sc, cy-h/2-2*sc,
                  cx-w/2+3*sc, cy+h/2+2*sc,
                  fill=CINZA_CLARO, width=int(1.5*sc))
...
def _arte_caminhao_leve(c, cx, cy, sc, cor):
    """Caminhão leve / VUC — cabine curta"""
    w,h = 36*sc, 16*sc
    cab = 10*sc   # largura da cabine
    # Baú
    c.create_rectangle(cx-w/2+cab, cy-h/2, cx+w/2, cy+h/2,
                       fill=_escurecer(cor), outline=BRANCO, width=1)
    # Cabine
    c.create_rectangle(cx-w/2, cy-h/2, cx-w/2+cab, cy+h/2,
                       fill=cor, outline=BRANCO, width=1)
    # Para-brisas
    c.create_polygon(cx-w/2+1*sc,  cy-h/2+2*sc,
                     cx-w/2+cab-1*sc, cy-h/2+2*sc,
                     cx-w/2+cab-1*sc, cy+h/2-2*sc,
                     cx-w/2+1*sc,     cy+h/2-2*sc,
                     fill="#A0C8F0", outline="")
    # Rodas (6 = toco)
    rw,rh=4*sc,5*sc
    for rx,ry in [(-w/2+2*sc,-h/2-rh/2),(-w/2+2*sc,h/2-rh/2),
                  ( w/4,     -h/2-rh/2),( w/4,      h/2-rh/2),
                  ( w/2-4*sc,-h/2-rh/2),( w/2-4*sc, h/2-rh/2)]:
        c.create_rectangle(cx+rx-rw/2,cy+ry,cx+rx+rw/2,cy+ry+rh,
                           fill="#222",outline="#555",width=1)

def _arte_caminhao_truck(c, cx, cy, sc, cor):
    """Caminhão truck — baú longo"""
    w,h = 48*sc, 18*sc
    cab = 10*sc
    # Baú longo
    c.create_rectangle(cx-w/2+cab, cy-h/2, cx+w/2, cy+h/2,
                       fill=_escurecer(cor), outline=BRANCO, width=1)
    # Linhas de painel no baú
    for lx in [cx-w/2+cab+8*sc, cx-w/2+cab+16*sc, cx-w/2+cab+24*sc]:
        c.create_line(lx, cy-h/2+1, lx, cy+h/2-1,
                      fill=CINZA_ESCURO, width=1)
    # Cabine
    c.create_rectangle(cx-w/2, cy-h/2+1*sc,
                       cx-w/2+cab, cy+h/2-1*sc,
                       fill=cor, outline=BRANCO, width=1)
    # Para-brisas
    c.create_polygon(cx-w/2+1*sc,     cy-h/2+3*sc,
                     cx-w/2+cab-1*sc, cy-h/2+3*sc,
                     cx-w/2+cab-1*sc, cy+h/2-3*sc,
                     cx-w/2+1*sc,     cy+h/2-3*sc,
                     fill="#A0C8F0", outline="")
    # Rodas (8)
    rw,rh=4*sc,6*sc
    for rx,ry in [(-w/2+2*sc,-h/2-rh/2),(-w/2+2*sc,h/2-rh/2),
                  ( 0,       -h/2-rh/2),( 0,        h/2-rh/2),
                  ( w/4,     -h/2-rh/2),( w/4,      h/2-rh/2),
                  ( w/2-4*sc,-h/2-rh/2),( w/2-4*sc, h/2-rh/2)]:
        c.create_rectangle(cx+rx-rw/2,cy+ry,cx+rx+rw/2,cy+ry+rh,
                           fill="#222",outline="#555",width=1)

def _arte_caminhao_carreta(c, cx, cy, sc, cor):
    """Carreta / bitrem — cavalo + semirreboque"""
    w,h = 56*sc, 17*sc
    cab = 10*sc
    # Semirreboque
    c.create_rectangle(cx-w/2+cab+4*sc, cy-h/2, cx+w/2, cy+h/2,
                       fill=_escurecer(cor), outline=BRANCO, width=1)
    # Linhas no reboque
    ...
    # Quinta roda / engate
    c.create_oval(cx-w/2+cab, cy-3*sc,
                  cx-w/2+cab+4*sc, cy+3*sc,
                  fill=CINZA_CLARO, outline="")
    # Cavalo (cabine)
    c.create_rectangle(cx-w/2, cy-h/2+2*sc,
                       cx-w/2+cab, cy+h/2-2*sc,
                       fill=cor, outline=BRANCO, width=1)
    c.create_polygon(cx-w/2+1*sc,     cy-h/2+4*sc,
                     cx-w/2+cab-1*sc, cy-h/2+4*sc,
                     cx-w/2+cab-1*sc, cy+h/2-4*sc,
                     cx-w/2+1*sc,     cy+h/2-4*sc,
                     fill="#A0C8F0", outline="")
    # Rodas
    rw,rh=4*sc,5*sc
    for rx,ry in [(-w/2+2*sc,-h/2-rh/2),(-w/2+2*sc,h/2-rh/2),
                  ( w/4-2*sc,-h/2-rh/2),( w/4-2*sc, h/2-rh/2),
                  ( w/2-4*sc,-h/2-rh/2),( w/2-4*sc, h/2-rh/2)]:
        c.create_rectangle(cx+rx-rw/2,cy+ry,cx+rx+rw/2,cy+ry+rh,
                           fill="#222",outline="#555",width=1)

def _arte_bicicleta(c, cx, cy, sc, cor):
    """Bicicleta vista de cima"""
    w,h = 16*sc, 5*sc
    # Quadro
    c.create_line(cx-w/2+2*sc, cy, cx+w/2-2*sc, cy,
                  fill=cor, width=int(2*sc))
    c.create_line(cx, cy, cx-w/6, cy-h/2,
                  fill=cor, width=int(1.5*sc))
    c.create_line(cx, cy, cx-w/6, cy+h/2,
                  fill=cor, width=int(1.5*sc))
    # Guidão
    c.create_line(cx-w/2+1*sc, cy-h/2-1*sc,
                  cx-w/2+1*sc, cy+h/2+1*sc,
                  fill=CINZA_CLARO, width=int(1.5*sc))
    # Rodas
    rr=3*sc
    c.create_oval(cx-w/2-rr/2, cy-rr/2, cx-w/2+rr/2, cy+rr/2,
...
def _arte_pedestre(c, cx, cy, sc, cor):
    """Silhueta humana vista de cima (posição andando)"""
    # Cabeça
    r_cab = 4*sc
    c.create_oval(cx-r_cab, cy-r_cab, cx+r_cab, cy+r_cab,
                  fill=cor, outline=BRANCO, width=1)
    # Tronco
    c.create_rectangle(cx-3*sc, cy+r_cab,
                       cx+3*sc, cy+r_cab+7*sc,
                       fill=cor, outline=BRANCO, width=1)
    # Braços (abertos levemente)
    c.create_line(cx-3*sc, cy+r_cab+2*sc,
                  cx-7*sc, cy+r_cab+5*sc,
                  fill=cor, width=int(1.5*sc))
    c.create_line(cx+3*sc, cy+r_cab+2*sc,
                  cx+7*sc, cy+r_cab+5*sc,
                  fill=cor, width=int(1.5*sc))
    # Pernas
    c.create_line(cx-2*sc, cy+r_cab+7*sc,
                  cx-3*sc, cy+r_cab+13*sc,
                  fill=cor, width=int(2*sc))
    c.create_line(cx+2*sc, cy+r_cab+7*sc,
                  cx+3*sc, cy+r_cab+13*sc,
                  fill=cor, width=int(2*sc))

def _escurecer(cor_hex):
    """Escurece levemente uma cor hex para detalhes."""
    try:
...
MODELOS_VEICULOS = {
    "carro": [
        {"chave":"sedan",  "nome":"Sedan",      "desc":"4 portas",       "arte":_arte_sedan, "png":"sedan.png", "larg":28,"alt":14,"cor":AZUL_CLARO},
        {"chave":"suv",    "nome":"SUV / Picape","desc":"4x4, utilitário","arte":_arte_suv,   "png":"suv.png",   "larg":30,"alt":16,"cor":"#3A8A3A"},
        {"chave":"hatch",  "nome":"Hatchback",  "desc":"Compacto",       "arte":_arte_hatch, "png":"hatch.png", "larg":24,"alt":13,"cor":"#A04040"},
    ],
    "moto": [
        {"chave":"esportiva","nome":"Esportiva","desc":"Moto baixa","arte":_arte_moto_esportiva,"larg":20,"alt":7,"cor":"#9B3030"},
        {"chave":"urbana",   "nome":"Urbana / Naked","desc":"Guidão alto","arte":_arte_moto_urbana,"larg":18,"alt":8,"cor":"#9B6BDF"},
        {"chave":"carga",    "nome":"Motoboy / Carga","desc":"Com baú","arte":_arte_moto_carga,"larg":22,"alt":9,"cor":"#3A6A3A"},
    ],
    "caminhao": [
        {"chave":"leve",   "nome":"Caminhão leve","desc":"VUC / 3/4","arte":_arte_caminhao_leve,"larg":36,"alt":16,"cor":COR_PERIGO},
        {"chave":"truck",  "nome":"Caminhão truck","desc":"Toco / truck","arte":_arte_caminhao_truck,"larg":48,"alt":18,"cor":"#C06020"},
        {"chave":"carreta","nome":"Carreta","desc":"Bitrem / semirreboque","arte":_arte_caminhao_carreta,"larg":56,"alt":17,"cor":"#606060"},
    ],
}

# ─────────────────────────────────────────────
#  ÍCONES E NOMES LEGÍVEIS POR TIPO
# ─────────────────────────────────────────────
TIPO_INFO = {
    "carro":     ("🚗", "Carro"),
    "moto":      ("🏍", "Moto"),
...
def gerar_elementos_modelo(icone, W, H):
    ...
    if icone == "cruzamento_mais":
        ...
        for ang in [90, 210, 330]:
            ...
            els.append({"tipo":"_asfalto","x":min(cx,x2)-v/2,"y":min(cy,y2)-v/2,
...
class SplashScreen(tk.Toplevel):
    def __init__(self, parent, img_brasao=None):
        ...
        if img_brasao and PIL_OK:
            ...
            tk.Label(corpo, text="⚖", font=("Segoe UI",52),
                     bg=AZUL_ESCURO, fg=AMARELO).pack(pady=(4,0))

        tk.Label(corpo, text="POLÍCIA CIENTÍFICA DO AMAPÁ",
                 font=("Segoe UI",10,"bold"), bg=AZUL_ESCURO, fg=AMARELO).pack(pady=(4,0))
        tk.Label(corpo, text="SICRO PCI/AP",
                 font=("Segoe UI",26,"bold"), bg=AZUL_ESCURO, fg=BRANCO).pack(pady=(6,0))
        tk.Label(corpo, text="Sistema de Croquis Periciais de Trânsito",
                 font=("Segoe UI",10), bg=AZUL_ESCURO, fg=CINZA_CLARO).pack(pady=(2,0))
        tk.Frame(corpo, bg=AZUL_MEDIO, height=1).pack(fill="x", pady=12)
        tk.Label(corpo, text="Departamento de Criminalística — DC",
                 font=("Segoe UI",9), bg=AZUL_ESCURO, fg=CINZA_CLARO).pack()
        tk.Label(corpo, text="Desenvolvido por André Ricardo Barroso · Perito Criminal",
                 font=("Segoe UI",9,"italic"), bg=AZUL_ESCURO, fg=CINZA_MEDIO).pack(pady=(3,12))
        ...
        tk.Label(self, text="v3.0.0", font=("Segoe UI",8),
    ...
    def _anim(self):
        if self._p < 100:
            ...
            msgs = {10:"Carregando interface…", 35:"Carregando biblioteca…",
                    60:"Preparando ferramentas…", 85:"Quase pronto…"}
...
class PopupModeloVeiculo(tk.Toplevel):
    """
    Grade de seleção de modelo de veículo.
    Mostra 3 modelos por categoria com preview desenhado no canvas.
    """
    def __init__(self, parent, tipo_base):
        ...
        # Garante foco e visibilidade ANTES do grab
        self.lift()
        self.focus_force()
        self.grab_set()
    ...
    def _card(self, parent, modelo, col, cw, ch):
        ...
        # Preview canvas
        prev = tk.Canvas(card, width=cw - 16, height=105,
        ...
        # Bind em todos os widgets do card
...
class PopupPlacas(tk.Toplevel):
    ...
    def _card(self, parent, m, row, col, cw, ch):
        ...
        self._desenhar_placa_preview(prev, m, (cw-16)//2, 35)

        tk.Label(card, text=m["desc"], font=("Segoe UI",8),
                 bg=COR_CARD, fg=CINZA_CLARO,
                 wraplength=cw-16).pack()
    ...
    def _desenhar_placa_preview(self, c, m, cx, cy):
        r = 22
        cor = m["cor"]
        label = m["label"]
        # Forma octogonal para PARE, circular para velocidade, triangular para atenção
        if m["chave"] == "pare":
            # Octógono
            pts=[]
            import math
            ...
            c.create_text(cx,cy,text=m["label"],fill="#CC0000",
                         font=("Segoe UI",11,"bold"))
...
class AppSicro(tk.Tk):
    def __init__(self):
        super().__init__()
        self.withdraw()
        self.title("SICRO PCI/AP — Polícia Científica do Amapá")
        self.configure(bg=COR_FUNDO)
        self.state("zoomed")
        ttk.Style(self).theme_use("clam")
        self._frame=None; self._brasao=None
    ...
    def _pos_splash(self,splash):
        splash.fechar(); self.deiconify(); self.mostrar_inicio()
    ...
    def _novo(self, modo):
        ...
        if modo == "drone":
            ...
            cam = filedialog.askopenfilename(
                title="Foto do drone (já retificada)",
        ...
        # Fechou sem escolher → trata como canvas livre
    ...
    def _abrir(self,caminho):
        ...
        editor.k=dados.get("k"); editor.u_k=dados.get("u_k")
        if editor.calibrado and editor.k:
            editor.label_escala.config(
                text=f"📐 k={editor.k*1000:.3f}mm/px",fg=COR_SUCESSO)
        editor._redesenhar()
        self._trocar(editor)

    def _trocar(self,novo):
        if self._frame: self._frame.destroy()
        self._frame=novo; novo.pack(fill="both",expand=True)
